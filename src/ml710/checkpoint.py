...
def init_model_with_materialized_weights(model, model_config, save_dir):
    ...
    if os.path.exists(index_path): # Handle sharded checkpoint
        ...
        for sft_name in layer_names:
            ...
            with safe_open(shard_path, framework="pytorch", device="cpu") as f:
                ...
                tensor = initialization_manager.adjust_tensor_size(tensor, sft_name)
    ...
    if pgm.process_group_manager.pp_is_last_stage or not model.__class__.__name__ == "PipelineParallel":
        ...
        if pgm.process_group_manager.tp_world_size > 1:
            ...
            state_dict['lm_head.weight'] = torch.zeros(vocab_per_rank, model_config.hidden_size)
        else:
            ...
            model.lm_head = FinalProjection(model_config.hidden_size, vocab_size, bias=False)
            state_dict['lm_head.weight'] = torch.zeros(vocab_size, model_config.hidden_size)
